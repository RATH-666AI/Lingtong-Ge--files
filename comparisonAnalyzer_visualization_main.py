...
class CompleteComparisonAnalyzer:
    """
    SEPER与RAGAS评估结果完整对比分析器

    功能:
    1. 加载SEPER和RAGAS的评估结果
    2. 按问题对齐两个评估结果
    3. 生成多种可视化图表进行对比分析
    4. 计算相关性矩阵和统计摘要
    """
    ...
    def __init__(self, seper_results_path: str, ragas_results_path: str):
        ...
        Args:
            seper_results_path: SEPER评估结果JSON文件路径
            ragas_results_path: RAGAS评估结果JSON文件路径
        """
    ...
    def align_results(self) -> pd.DataFrame:
        """
        按问题对齐SEPER和RAGAS的评估结果

        Returns:
            对齐后的DataFrame，包含两套评估体系的指标
        """
    ...
    def _safe_polyfit(self, x, y, deg=1):
        ...
        修复: 原代码中使用np.polyfit时遇到NaN或数据点不足会抛出
    ...
    def _plot_scatter_comparisons(self, save_path: str):
        ...
        展示ΔSEPER与其他指标之间的关系
        修复: 使用_safe_polyfit替代直接调用np.polyfit
        """
